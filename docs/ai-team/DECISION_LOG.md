...
## 2026-07-14 — SERVICES統合（カード＝相談リンク化）＋「AI生成感」削減の第一波
オーナー指摘「入口カードがクリックできると分からない」「サイト全体にAIで作った感がある。ルール・トンマナは守る。余白は哲学として見せるが平坦にはしない」。brand-philosopher の全体監査 → メイン実装 → frontend-engineer クロスレビュー。

- **SERVICES統合**: 入口チップ帯（entry-card 3枚）を廃止し、サービスカード3枚自体を `chat.html?t=` へのリンクに。チップの文言はカード下部の**下線つきCTA行**（「サイトや資料を整えたい →」等・margin-linkと同じ既存アフォーダンス）として継承。前回「導線を消すため見送り」としたdesign-critic案を、オーナーのクリック可視性指摘により採用。円形アイコンバッジ（.sv-ic＝SaaSテンプレの型）とピルタグ（.sv-tags）は廃止、コーナーラベルはaccent-inkのserif eyebrowへ
- **ラベル刷新**: `.s-lbl` のピル型（999px角丸＋薄桃背景）を廃止 → 短い罫（24px線）＋serif和語の素の文字に。英語monoラベル5つを和語化: // SERVICES→**手のうち** / WAYS OF WORKING→**関わり方** / SECURITY & AI→**守る線** / FAQ→**問い** / BLOG→**書きもの**（既存の「兆し」「余白の地図」の方向性を完遂）。blog-prev-badgeのピルも素の文字に
- **brand-philosopher監査の要点（AI生成感の三大要因）**: ①「3の暴政」＝3カラムグリッド6回＋同一セクション型の反復（余白が平坦に見える正体は量でなく緩急の欠如）②ピル・円バッジ・//ラベル・ガラスカードというAIビルダー部品語彙 ③汎用部品（ROIウィジェット型チェッカー・粒子演出・SCROLLヒント）の混入
- **残タスク（第二波候補・M/L工数）**: sign-grid等の非対称化（緩急の設計）／Vision粒子・光線を「紙と光」の語彙へ描き直し／余白の地図への手描き一点物／case-tagピル／EN版の意匠追従（ピル・タグ・英語ラベルが旧デザインのまま＝JAと乖離。cross-review指摘）／SCROLLヒントの扱い
- **守った資産（philosopher指定・消してはいけないもの）**: 時刻連動ヒーロー光／「4つ目を、探しています。」／consoleの手紙／ft-note「私ひとりの事務所です」／byline／FAQの一人称回答／margin-mapの非対称と左罫

## 2026-07-14 — 余白の地図・SERVICES のデザインブラッシュアップ
オーナー指示（スクリーンショット指定）。visual-designer＋design-critic の2体監査 → メイン裁定・実装 → frontend-engineer クロスレビュー。

- **余白の地図**: 方眼を可視化（.035→.06）・埋もれて誰にも見えなかった中央線`::before`を削除・ステップカードを「--bgベタ＋サーモン上罫1px」へ（小さな診断カードの左罫と呼応する罫線の語彙）・min-height 184px廃止・非リンクのhover反応を削除・番号と見出しの距離を28→20px
- **SERVICES**: `.svc-bento` を2+1→**等幅3カラム**（横長カードは右半分が空洞で根拠なし。769–1100pxは縦積み）・`.sv-wide`廃止・非リンク`.sv`のhover削除（「押せないものが反応する」嘘の解消）・登場モーションをYに統一・和文コーナーラベルをmono→serif 11px・タグを10→11px（コントラストは既存AA）・entry-cardに左罫2px（入口リンクの印）＋帯下マージン32pxで二段の役割を分化・sv4の冗長タグ「業務整理」削除・`.sv-t`に text-wrap:balance＋word-break:auto-phrase（文節折返し・非対応環境は従来挙動）
- **不採用の裁定**: design-critic の「entry-card段の全削除・svカードへの統合」は、オーナー承認済みの chat.html?t= 導線（2026-07-03決定）を消すため見送り（役割の視覚分化で冗長感に対処）。「箱3つ廃止して一本の線の地図化」は改修規模と退行リスクからブラッシュアップの範囲外として保管。「sv-tags全削除」も情報価値を優先し格上げに留めた
- cross-review指摘（hover時に左罫が薄まる・死にセレクタ3箇所）は全て修正済み

## 2026-07-14 — ホーム: 余白の間奏セクション削除・ヒーローbylineの地名削除
オーナー指示。
- `#s-interlude`（「この、なにもない場所が、余白です。」2026-07人間味監査Phase2で新設）を**削除**。関連CSS・モバイルmedia query・noscript参照・scrollspyの`s-interlude`も掃除
- ヒーローbylineから「いまはチェンマイの机から。」を削除（デジタルノマドは所在が定まらず、チェンマイを誇張する意図もないため）
- **追記（同日）**: About下スライダーの「チェンマイを拠点に、旅しながら働いています。」も削除で確定（オーナー回答=A）。5枚→4枚。JS/CSSはスライド枚数非依存の縦積みリストのため、削除のみで実装完了
...
- **人手の事前レビューを外す判断だが、自動生成側の安全網は維持**: 出典はホワイトリスト登録済みRSSの実URLのみ／`validateLLMResult` がid捏造・本文へのURL混入・空フィールドで停止／候補が `minItems` 未満の日は投稿しない（fail-safe）。構造的に「存在しないニュース・デタラメなリンク」は出ない。残留リスクは要約・前後文の言い回しが無レビューで公開される点のみ（オーナー了承のトレードオフ）
- **オーナーしかできない前提（コード側では不可能）**: リポジトリ Secret `ANTHROPIC_API_KEY` の設定。未設定ならモードに関係なくworkflowは何もせず正常終了する。gh CLI がこの環境に無く、シークレット/変数はコミット不可のため、この1点だけはGitHub UIでの設定が必要
- 戻すには Variables に `NEWS_MODE=draft` を追加するだけ（コード変更不要）

## 2026-07-13 — 新要素5点の一括導入（季節/404/紙のうえの光/月アニメ/雨の音）・罫線は見送り
オーナー承認（提案7案中「2・3・5・6・7以外」）。3分隊並列実装（ファイル分割で衝突回避）→ メイン統合 → a11y-auditor クロスレビュー → 修正。

- **season.html 新設「いまの季節」（七十二候）**: sky.htmlの太陽黄経（Meeus略式）を移植し、外部APIなしで今日の候を計算。判定は23:59時点（正午判定は+1日の系統誤差を計測により発見・修正）で**2026年の二十四節気24件がNAOJ暦要項と完全一致**。72候の漢字・読みはNAOJ暦Wiki（宝暦暦以降＝略本暦系統）を一次資料に二次照合済み。yohaku索引に「03 いまの季節」（ライブ状態=今日の候名）を追加し全5項目に
- **404.html 新設「ここは、余白です。」**: JSなし・noindex・sitemap非掲載。リンクは絶対パス `/fugasha_ai/`（GH Pagesの404は任意の深さで配信されるため）
- **index.html「紙のうえの光」**（保管案Aの実装）: ヒーロー背景のマウス追従を廃止し、実時刻の光（アンカー7点の連続補間・10秒呼吸・α≤0.10を全時刻走査で実証）へ。reduced-motion時は静的な時刻グラデ1回描画（従来の無描画から改善）
- **sky.html「月が、今夜まで満ちる」**（保管案の実装）: 初回のみ新月→今夜の月相へ2.4s ease-out。終点=計算結果そのもの。reduced-motion/hidden/画面外は即最終形。fail-open維持
- **focus.html「雨の音」**: WebAudioでその場合成（ピンクノイズ+highpass/lowpass+粒bandpass）・音源ファイルなし・**完全オプトイン**（AudioContextはクリック時のみ生成・再生状態は永続化しない）・非対応環境ではUI自体を出さない
- **「罫線が先に読む」は見送り**: 既存のページ進捗バー`.pb`とVisionの`.sst-progress-bar`が既にあり、3本目の線は「どの項目も改善しない演出は実装しない」に該当（creative-coder裁定・メイン同意）
- **a11yクロスレビューでの修正**: 重大1=夜間帯にヒーロー小テキスト（--hint/--muted）がcanvas重畳で4.5:1割れ → `.h-actions-note`/`.h-byline-txt`を--fg2へ（最悪重畳7.6:1）。中1=seasonの「いま」印を不可視テキスト化＋擬似要素●に空代替テキスト。ほか軽微4件（nxHeadのh2化・ring-wrapのnoscript余白・雨説明文のaria-describedby・heroBg canvasのaria-hidden）すべて修正済み
- **既存バグ修正**: focus.htmlのグローバルkeydownがbutton/a/summary上のSpaceを奪いタイマーが誤作動 → ネイティブ押下を優先する除外を追加
