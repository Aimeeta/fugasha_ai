...
## 2026-07-19 — MARGIN MAP簡素化・コピー刷新・Less friction独立セクション化
オーナー指摘「3ステップ盤と小さな診断は削除」「余白は探すものではなく〜等の言い回しが硬い。可視化します＋伴走の流れに」「小さな試算は英語に（試算のアイデア自体は良い）」「SERVICESのタイトルが弱い。カードにアイコンを」「関わり方のコピーが不自然」「Less friction, More curiosityをAboutから独立させ、FAQ直前に大きく・アニメつきで。CTAボタンも」。メイン実装。

- **MARGIN MAP**: margin-map一式（小さな診断ノート＋方眼盤＋書き出す/分ける/戻す3ステップ）を撤去し、チェッカー中心の構成へ。見出し「忙しさの正体を、一枚の地図にします。」／desc「どこで詰まっていて、どこを手放せるのか。〜可視化するところから始めます。答えを急がず、伴走しながら。」（オーナー指定の語彙「可視化」「伴走」を採用 — BRAND_PRINCIPLESのコンサル用語回避よりオーナー指示を優先）。kicker「小さな試算」→ **QUICK ESTIMATE**（.mck-k新設）。リードに「一緒に整理してみる →」リンクを追加（旧margin-noteのCTAを継承）
- **SERVICES**: 見出し「やっていることは、この3つです。」→**「バラバラに頼まなくて、いい。」**（ワンストップの魅力を一言で）。3カードに sign-ic と同言語のストロークアイコン（鉛筆/スパークル/レイヤー、.sv-ic）を追加
- **WAYS OF WORKING**: 見出し「関わり方は、つくる・学ぶ・一緒に進める。」→**「頼み方は、ひとつじゃありません。」**／descを「つくって納品する。手を動かしながら教える。隣で一緒に進める。〜」に
- **#s-catch 新設**（About→FAQ間）: 「Less friction. / More curiosity.」を clamp(40px,6.4vw,80px) のserifで中央表示。2行目はaccent-inkイタリック。語ごとの浮上＋blur解除のスタガーリビール＋遅延CTAボタン（相談メモをつくってみる→chat.html）。Aboutの.about-catchは撤去。sectionIds・noscriptに追加済み
- 検証: タグ整合・node --check・コンソールエラーなし。デスクトップ/モバイル375で新コピー・アイコン・catchセクションの描画確認（ブラウザHTTPキャッシュに注意 — 確認時は ?v= クエリで回避）

## 2026-07-19 — セクションラベル英語回帰・視覚要素の追加・モバイル横スライダー化
オーナー指摘「和語ラベルは硬い（英語が良かった）」「兆しが文章だらけで無機質。アイコンか画像を」「イラストやグラデーションで美しさを」「スクロールが長い。スライダー等で圧縮を」「哲学の罫線と見出しの間が狭すぎる」。メイン実装。

- **ラベル英語化（JA/EN共通）**: 2026-07-14の和語化を撤回。s-lbl を SIGNS / MARGIN MAP / SERVICES / WAYS OF WORKING / SECURITY & AI / FAQ / BLOG、tools-lbl を TOOLS WE THINK WITH に。スタイルは現行の「短い罫＋serif」を維持し、EN側の旧ピル型（mono＋999px角丸）もこのスタイルへ移植して両言語で統一。EN本文の `// ` プレフィックスは全廃
- **兆しにアイコン**: sign-item 3枚に Tabler系ストロークSVG（時計・鉛筆・スパークル、26px・accent-ink・丸バッジなし=AIビルダー語彙を避ける）を追加
- **グラデーション**: sec-white をベタ→縦の淡いグラデ、#s-philosophy に夜明けのradial（上辺）、#s-cta に残り火のradial（下辺・α.13）。phi-head 直下に「地平線と昇る太陽」のインラインSVGオーナメント（phi-mark・グラデ塗り）を新設
- **スクロール削減**: sst-outer 280→240vh（モバイル320→260vh）。モバイル(≤768px)で svc-bento / cases-grid / blog-prev-grid を横スクロール＋scroll-snapのスライダーに（カード幅84%で次カードを覗かせる。ネイティブスクロールのみ＝2026-07-03のホイール奪取禁止に抵触しない）。cases-grid はリンク非内包のため tabindex=0＋focus-visible輪郭でキーボード対応
- **罫線と見出しの余白**: phi-item のアクセント線→見出し間を実質0px（負マージンで相殺されていた）→34pxに修正。モバイルの sec::before を top:16px に上げてラベルとの間隔を確保
- 検証: 両ページ node --check・タグ整合、モバイル375で各セクション描画・スライダーのはみ出し幅、デスクトップ1280でグリッド復帰をDOM計測で確認（ブラウザペインのスクリーンショットが1操作遅れで追従する不具合があり、一部はDOM計測での確認）

## 2026-07-19 — ヒーロー1カラム化＋業務時間チェッカーを「余白の地図」へ移設
オーナー指摘「ヒーロー右コラムの情報量が多すぎる。チェッカーは別セクションへ。左コラムを綺麗に並べ、AIで作った感を減らす」。メイン実装（小タスクのため単独）。

- **チェッカー移設**: ヒーロー右コラムを廃止し、`#s-margin`（余白の地図）末尾へ `.margin-checker`（左リード＋右カード、margin-mapと同じ .92/1.08 グリッド）として移設。「手放せる時間」の話は余白の地図の文脈に属するため。リード見出し「手放せる時間を、ためしに数字にしてみる。」（kicker: 小さな試算）
- **ダッシュボード語彙の除去**: アプリ風タイトルバー（mono大文字「業務時間チェッカー」＋ドット `.sld`）を廃止し、margin-stepと同じ「--bgベタ＋サーモン上罫」の紙カードへ。バーの常時アニメ `bar-breathe` も削除（静けさ優先）。ガラス＋backdrop-filterも不要になり削除
- **ヒーロー1カラム化**: グリッド2カラム→ブロック1カラム。並び順を h1→subcopy→**byline（Aimee写真を先に＝人の気配をCTAより前へ）**→CTA→note→choice-label→チップ3枚横並び（900px上限、モバイルは縦積み）に変更
- **AIビルダー部品の削除**: チップの番号サークル（01/02/03 mono丸囲み）廃止・SCROLLヒント（mono英字＋アニメ線）廃止（第一波の残タスク消化）
- **副修正**: チェッカー初期化を `updateChecker(15)` 固定→スライダー実値参照に（ブラウザのフォーム値復元と表示がずれるバグの予防）
- 検証: node --check（inline JS）／タグ整合カウント／localhost で desktop 1280（DOM計測: 2カラム整列・チップ3列）・tablet・mobile 375 表示＋スライダー動作・SR通知・コンソールエラーなしを確認
