...
| 項目 | 方法 | 結果 |
|---|---|---|
| inline JS 構文 | 全6ファイル `node --check`（JSON-LDは別途JSON検証） | ✅ OK |
| タグ整合 | 主要タグ開閉カウント（全ファイル） | ✅ 全一致 |
| 二十四節気の正確性 | 2026年24件をNAOJ暦要項と照合（node+ブラウザ実測） | ✅ 完全一致（23:59判定に修正後） |
| 72候データ | NAOJ暦Wiki（宝暦暦以降）を一次資料に、便利帳で二次照合 | ✅ 72件一致 |
| 紙のうえの光 α上限 | heroLight() を-3h〜27h全走査 | ✅ 最大α=0.1000・色域はパレット凸包内・境界連続 |
| 月アニメ | moonPath抽出の単体検証15項目＋実ブラウザ最終形 | ✅ 全PASS・終点=即時描画と一致 |
| 雨の音 | 実クリックでON/OFF・aria-pressed連動・保存/復元・EN/JA | ✅ 動作（聴感は未検証・下記） |
| a11yクロスレビュー | a11y-auditor（実測込み・重大1/中1/軽微5指摘） | ✅ 全件修正済み（夜間コントラストは--fg2化で7.6:1） |
| キーボード | Space奪取修正: button上→ネイティブ生存 / body上→タイマー起動を合成イベントで実測 | ✅ 副作用なし |
| コンソール | index/season/404/yohaku/sky/focus | ✅ エラーゼロ |
| モバイル375 | season/404 横溢れ実測・雨UIレイアウト | ✅ 溢れなし |

### 未検証領域（開示）
- 雨の音の**聴感**（合成パラメータは机上設計。実機で音量・質感の確認を推奨）
- reduced-motion のOS実設定トグル／JS無効ブラウザの実挙動（コード経路・パターン検証のみ）
- 実スクリーンリーダー（VoiceOver/NVDA）通し読み上げ
- 404.html のリンクは本番パス `/fugasha_ai/` 前提（ローカルでは飛べない仕様）
- season.html は端末タイムゾーン依存（JST以外で±1日ずれうる。ページ内開示済み）
...
| 項目 | 方法 | 結果 |
|---|---|---|
| inline JS（mainブロック抽出） | `node --check` | ✅ OK |
| タグ整合 | body側マークアップ開閉カウント | ✅ 全一致 |
| コンソール | localhost:8901 | ✅ エラーゼロ |
| ライブ状態 | 01集中/いま/09:22(ドット有)・02今日/7月13日・03今夜の空/二十九日月・04ことば/—著者 | ✅ 各ページと整合・ドットは時計行のみ |
| デスクトップ1280 | 横溢れ判定・目視 | ✅ 溢れなし・字間/連番/レイアウト良好 |
| モバイル375 | 横溢れ・1カラム時の連番配置 | ✅ 溢れなし・連番は編集的マーカーとして自然 |
| スキップリンク | Tabでskip-linkにoutline可視・飛び先#index実在 | ✅ 機能 |
| **フォーカス指標(2.4.11)** | キーボードでentryにTab→`outline:2px solid --accent-ink`可視・:focus-visible=true | ✅ **a11y退行を修正済み** |
| aria-label削除 | entryのaria-label=null（可視テキストが読み上げ名） | ✅ 情報等価 |
| reveal fail-open | .reveal 5/5・html.js除去時opacity=1 | ✅ 維持 |
| a11y cross-review | a11y-auditorがWCAG2.2で6項目検証（コントラスト実測） | ✅ 5合格＋2.4.11のみ指摘→修正で解消 |

### コントラスト実測（a11y-auditor・背景#efe4d3基準）
.val/.role(--fg2) 9.41:1・連番/ラベル/EN(--hint) 6.26:1・フォーカスoutline(--accent-ink) 6.45:1・skip-link 13.4:1 — いずれもAA合格。live-dot(2.08:1)はaria-hidden純装飾で対象外。
...
| 項目 | 方法 | 結果 |
|---|---|---|
| inline JS 構文 | `node --check` | ✅ OK |
| 当日(7/13)表示 | ブラウザDOM: 見出し「この日、世界では」/ 1985 ライヴエイド開催 / 空ノート非表示 | ✅ 正 |
| フォールバック選定 | node シミュレーション 7/2・7/28・7/31（日付なし日） | ✅ 近い順に実在4件・日付つき・空にならない |
| 7月日別カバレッジ | イベント日の抽出 | ✅ 21件/18日に分散（12日は4件） |
| コンソール | today.html | ✅ エラーゼロ |

未検証: フォールバック表示の実ブラウザ目視（プレビューのスクロール再描画制約のため。選定はnode・描画は文字列連結で自明）。他月の日別密度は当月比で低い（開示）。
